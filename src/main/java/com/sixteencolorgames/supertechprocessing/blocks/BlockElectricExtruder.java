...
public class BlockElectricExtruder extends BlockBase {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (playerIn.isSneaking()) {
            ...
            if (!worldIn.isRemote) {
                if (worldIn.getTileEntity(pos) instanceof TileEntityElectricExtruder) {
                    playerIn.openGui(SuperTechProcessingMod.instance, GUIHandler.EXTRUDER_ELECTRIC_GUI, worldIn, pos.getX(), pos.getY(), pos.getZ());
                }
            }
        }
        return true;
    ...
    public TileEntity createNewTileEntity(World worldIn, int meta) {
        return new TileEntityElectricExtruder();
    }
    ...
    public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        ...
        if (stack.hasDisplayName()) {
            ...
            if (tileentity instanceof TileEntityElectricExtruder) {
                ((TileEntityElectricExtruder) tileentity).setCustomInventoryName(stack.getDisplayName());
            }
        }
    }
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        if (!keepInventory) {
            ...
            if (tileentity instanceof TileEntityElectricExtruder) {
                InventoryHelper.dropInventoryItems(worldIn, pos, (TileEntityElectricExtruder) tileentity);
